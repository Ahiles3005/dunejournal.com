...
class NewsController extends Controller
{
    ...
    public function info(){
        ...
        if($news == null) return jsonResponse( ['error' => 'Новость не существует!'] );
    ...
    public function add(){
        ...
        if( empty($this->request->tags) ) return jsonResponse( ['error' => 'Вы должны указать хотя бы один тэг!'] );
        if( empty($this->request->categories) ) return jsonResponse( ['error' => 'Вы должны указать хотя бы одну тематику!'] );

        $assetUrl = null;
        if( isset($this->request->asset) ){
            $filename = 'asset-'.time().'-'.Str::random(24). '.'. $this->request->asset->getClientOriginalExtension();
        ...
        $fullImage = null;
        if( isset($this->request->full_image) ){
            $filename = 'full-asset-'.time().'-'.Str::random(24). '.'. $this->request->full_image->getClientOriginalExtension();
        ...
        if($saved) {
            ...
            if( isset($this->request->gallery) ) $this->saveGallery($this->request->gallery, $saved->id);
    ...
    public function edit(News $newsObject){
        ...
        $news = News::find($this->request->id);
        if($news == null) return jsonResponse( ['error' => 'Новость не существует для редактирования!']);
        ...
        if( isset($this->request->asset) ){
            ...
            if($stored){
                $assetUrl = url('') . '/storage/news_assets/'.$filename;
                $data['asset_url'] = $assetUrl;
                if($currentAsset != null) $newsObject->clearAsset($currentAsset);
            }
        } else if( intval($this->request->del_asset) == 1 && $currentAsset != null) {
        ...
        if( isset($this->request->full_image) ){
            ...
            if($stored){
                $fullImage = url('') . '/storage/news_assets/'.$filename;
                $data['full_image'] = $fullImage;
                if($currentFullImage != null) $newsObject->clearAsset($currentFullImage);
            }
        } else if( intval($this->request->del_full_image) == 1 && $currentFullImage != null) {
        ...
        if($updated) {
            ...
            if( isset($this->request->gallery) ) $this->saveGallery($this->request->gallery, $news->id);
        ...
        return jsonResponse( ['error' => 'Не удалось обновить новость. Перепроверьте входные данные и повторите попытку!']);
    }

    private function saveGallery($gallery, $newsId){
        foreach($gallery as $key => $value){

            $filename = 'gallery_asset-'.microtime().'-'.Str::random(3). '.'. $value->getClientOriginalExtension();
